  ...
  new HttpRequester("get_chats").callApi().then((res: any) => {
    if (res?.success) {
      ...
      for (let i = 0; i < chats?.length; i++) {
        ...
        rooms.value.push({
          roomId: chats[i]?.id,
          roomName: chats[i]?.course_title + ' ' + chats[i]?.student_name,
...
const sendMessage = (message: any) => {
  ...
  router.push(`/instructorChats/${message?.roomId}`);
...
<template>
  <Nanbar />
  <div class="container">
    <InstructorSideBar selected="Chats" class="sidebar" />
    <div class="wrapper">
      ...
      <vue-advanced-chat
        ...
        @fetch-messages="
          router.push(`/instructorChats/${$event.detail[0].room.roomId}`)
        "
      />
