...
if (prop.materialId) {
  new HttpRequester("get_simulation")
    .callApi({ simulation_id: prop.materialId })
    .then((res) => {
      if (res?.success) {
        title.value = res?.data?.simulation?.title;
        description.value = res?.data?.simulation?.description;
        source.value =res?.data?.simulation?.source;
        image.value=res?.data?.simulation?.image;
      }
    });
}
watch(prop, () => {
  if (prop.addMaterial) {
    ...
    if (prop.materialId) {
      let data = {
        simulation_id: prop.materialId,
        update: mat,
      };
      new HttpRequester("update_simulation").callApi(data).then((res) => {
        if (res?.success) {
          emit("info", null);
        }
      });
      ...
      new HttpRequester("create_simulation").callApi(data).then((res) => {
        if (res?.data?.simulation_id) {
          let info = {
            Id: res?.data?.simulation_id,
