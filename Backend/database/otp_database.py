...
async def create_otp(email: str):
   ...
   # link = f"https://www.traceedtech.com/#/forgotPassword/{access_token}"
    ...
    send_password_reset_email(reset_link=link, to_email=email, otp_code=otp_code)
...
async def reset_password(password: str, token: str):
    ...
    try:
        ...
        await get_database().get_collection('user').update_one({'_id': bson_id}, {'$set': {'is_verefied': True}})
